...
const TeachOnPage = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (formData) => {
      ...
      await axios
        .post(
          "https://edu-manage-website-server.vercel.app/teacherRequests",
          formData
        )
        ...
        .then((res) => {
          ...
          if (res.data.insertedId) {
            ...
            setMessage(
              "Your Response have been recored. Your Application is Under Admin Review"
            );
          }
        });
    },
  ...
  const { refetch, data: teacherReq = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(
        `https://edu-manage-website-server.vercel.app/teacherRequests/${user.email}`
      );
      console.log(res.data);
      ...
      if (teacherReq.status === "accepted") {
        ...
        setFormVisibility(false);
        setMessage(
          "Sorry!! Unfortunately Your Teacher Request Has Been Rejected"
        );
        setReqbtn(true);
      }
  ...
  const handleResubmit = () => {
    axios
      .patch(
        `https://edu-manage-website-server.vercel.app/teacherRequests/pending/${user.email}`
      )
      .then((res) => {
        console.log(res.data);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold my-5 text-pink-600 font-lato">
        Become a Part of EASYLEARN
      </h1>
      {formVisibility ? (
        <div className="w-3/4 mx-auto bg-gray-50 dark:bg-gray-700 dark:text-white p-7 my-10">
          <h3 className="text-left mb-3 text-blue-600 dark:text-amber-100">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 ">
            ...
            <div className="flex gap-3 flex-col lg:flex-row">
              ...
              <label className="form-control w-full ">
                ...
                >
                  ...
                  <option value="Academic Instructor">
                    {" "}
                    Academic Instructor
                  </option>
            ...
            <button
              type="submit"
              className="flex gap-3 items-center bg-gradient-to-r from-[#447cb4] to-[#bf5aac] text-white font-semibold text-lg py-2.5 px-3 rounded"
            >
        ...
        <h1 className="text-3xl text-sky-400">{message}</h1>
      )}
