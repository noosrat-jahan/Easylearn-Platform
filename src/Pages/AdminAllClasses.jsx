...

                            {
                                newCreatedClass.map(newClass => <tr key={newClass._id} className="border-b border-opacity-20 dark:border-gray-300 dark:bg-gray-50">
                                    ...
                                    <td className="p-3 text-center flex gap-2 justify-center items-center">
                                        <Button
                                            disabled={newClass.status === 'accepted' || newClass.status === 'rejected'} onClick={() => { handleMakeTeacher(newClass) }} className="px-5 py-2.5 btn font-semibold rounded-md bg-green-600 text-gray-50"
                                        >Approve</Button>
                                        <Button
                                            disabled={newClass.status === 'accepted' || newClass.status === 'rejected'}
                                            onClick={() => { handleRejectTeacher(newClass) }}
                                            className="px-5 py-2.5 btn font-semibold rounded-md bg-orange-600 text-gray-50"
                                        >Reject</Button>

                                        <Link to={`/studentdashboard/teacherclasses/${1}`} variant='solid' className='bg-[#441752] text-sm font-semibold text-white px-2 py-1 rounded-md' w={'full'} margin={'auto'}>
                                            See Progress
                                        </Link>
                                    </td>
                                </tr>)
